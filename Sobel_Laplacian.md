...
Т.е. наша картинка — это просто двумерная матрица чисел. Поэтому производная в области работы с изображениями — это отношение значения приращения пикселя по y к значению приращению пикселя по x.
```
dI = dy/dx
```
...
#### Теория ####
Используя первую производную интенсивности (оператор Собеля), мы смогли найти граничный пиксель (выделить некаторый контур). Это видно по "скачку" фукции произдводной:
